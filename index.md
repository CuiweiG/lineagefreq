...
Relative Rt estimates are consistent with published values: BA.2 = 1.34x
vs BA.1 ([Lyngse et
al. 2022](https://doi.org/10.1038/s41467-022-33498-0), published
1.3–1.5x); KP.3 = 1.36x vs JN.1. Generation times: 3.2 days for Omicron
...
*Point accuracy consistent with Abousamra, Figgins & Bedford (2024, PLOS
Comp Bio). MAE in percentage points. “…” = fill from `analysis/results/`
after running `analysis/run_all.R`. KS D = Kolmogorov–Smirnov distance
from PIT uniformity.*

**Calibration finding:** The 95% parametric prediction intervals cover
only 40–71% of observations — a systematic bias inherent to the MLR
...
prediction intervals demonstrate systematic miscalibration of parametric
prediction intervals and the effectiveness of conformal correction.

![](reference/figures/readme_performance_calibration.png)

### Fitness decomposition and multi-pathogen application

Immune-aware fitness decomposition separates intrinsic transmissibility
from immune escape. The same pipeline applies to influenza surveillance
data from WHO FluNet.

![](reference/figures/readme_fitness_influenza.png)
...
methods for fits, forecasts, and backtest summaries - Colorblind-safe
palettes
